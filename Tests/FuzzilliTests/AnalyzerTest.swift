...
struct AnalyzerTests {
    ...
    func testContextAnalyzer() {
        ...
        fuzzer.sync {
            ...
            for _ in 0..<10 {
                ...
                #expect(contextAnalyzer.context == .javascript)
            }
        }
    }
    ...
    func testObjectLiterals() {
        ...
        fuzzer.sync {
            ...
            #expect(b.context == .javascript)
            let v = b.loadInt(42)
            b.buildObjectLiteral { obj in
                #expect(b.context == .objectLiteral)
                obj.addProperty("foo", as: v)
                #expect(b.context == .objectLiteral)
                obj.addMethod("bar", with: .parameters(n: 0)) { args in
                    #expect(b.context == [.javascript, .subroutine, .method])
                }
                #expect(b.context == .objectLiteral)
    ...
    func testNestedLoops() {
        ...
        fuzzer.sync {
            ...
            let _ = b.buildPlainFunction(with: .parameters(n: 3)) { args in
                #expect(b.context == [.javascript, .subroutine])
                ...
                b.buildWhileLoop({
                    ...
                    #expect(b.context == [.javascript, .subroutine, .loop])
    ...
    func testNestedFunctions() {
        ...
        fuzzer.sync {
            ...
            b.buildAsyncFunction(with: .parameters(n: 2)) { _ in
                #expect(b.context == [.javascript, .subroutine, .async])
                let v3 = b.loadInt(0)
        ...
        fuzzer.sync {
            ...
            let superclass = b.buildClassDefinition { cls in
                cls.addConstructor(with: .parameters(n: 1)) { params in
                    #expect(b.context == [.javascript, .subroutine, .method, .classMethod])
                    b.buildDoWhileLoop(
                        do: {
                            #expect(
                                b.context == [
                                    .javascript, .subroutine, .method, .classMethod, .loop,
                                ])
            ...
            b.buildClassDefinition(withSuperclass: superclass) { cls in
                ...
                cls.addStaticInitializer { this in
                    #expect(b.context == [.javascript, .method, .classMethod])
                }
    ...
    func testCodeStrings() {
        ...
        fuzzer.sync {
            ...
            let _ = b.buildCodeString {
                ...
                b.buildRepeatLoop(n: 10) { _ in
                    b.loadInt(1337)
                    #expect(b.context == [.javascript, .loop])
                    let _ = b.buildCodeString {
        ...
        fuzzer.sync {
            ...
            let _ = b.buildPlainFunction(with: .parameters(n: 5)) { args in
                ...
                b.buildSwitch(on: args[4]) { swtch in
                    #expect(b.context == .switchBlock)
                    swtch.addCase(case1) {
                        #expect(b.context == [.javascript, .subroutine, .switchCase])
                    }
                    swtch.addCase(case2) {
                        #expect(b.context == [.javascript, .subroutine, .switchCase])
                    }
        ...
        fuzzer.sync {
            ...
            b.buildSwitch(on: case1) { outer_switch in
                ...
                outer_switch.addCase(case1) {
                    #expect(b.context == [.javascript, .switchCase])
