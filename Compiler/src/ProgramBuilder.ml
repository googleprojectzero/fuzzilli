...
let init_tracker emit_builtins include_v8_natives use_placeholder = {
    ...
    var_scope_map = Core.Map.empty (module Core.String);
...
let add_new_var_identifier_local name num glob_vis tracker =
    ...
    if glob_vis then
        ...
        let new_var_map = Core.Map.update tracker.var_scope_map name (fun _ -> fuzzilli_name_string) in
...
let lookup_var_name tracker name = 
    ...
    Some x -> InScope x
    | None -> match Core.Map.find tracker.var_scope_map name with
...
let build_begin_for_in_op var name tracker =
    ...
    add_new_var_identifier_local name left_var false tracker;
    Program_types.{
        inouts =  [var; left_var];
...
let build_begin_for_of_op var name tracker =
    ...
    add_new_var_identifier_local name left_temp false tracker;
    Program_types.{
...
let build_begin_catch_op name tracker =
    ...
    add_new_var_identifier_local name intermed_temp false tracker;
    Program_types.{
...
let id_to_func_type id tracker = 
    ...
    add_new_var_identifier_local id temp false tracker;
    (temp, _type)
...
let build_func_ops func_name_opt arg_names rest_arg_name_opt is_arrow is_async is_generator tracker =
    ...
    let rest_temp, rest_type = match rest_arg_name_opt with
        ...
        | Some rest_arg_name ->
            ...
            add_new_var_identifier_local rest_arg_name rest_var false tracker;
            [rest_var], [type_mess]
        in
