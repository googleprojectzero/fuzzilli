...
let correct =
    ...
    let left_for_in = get_new_intermed_temp tracker in
    let _, begin_for_in_inst = build_begin_for_in_op left_for_in arr_temp tracker in
    let reassign_inst = build_reassign_op string_temp left_for_in tracker in
    let end_for_in_inst = build_end_for_in_op tracker in
    let string_temp2, load_string_inst2 = build_load_string "D" tracker in
    let add_temp, add_inst = build_binary_op string_temp string_temp2 Plus tracker in
