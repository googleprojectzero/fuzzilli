...
let correct =
    ...
    let for_in_temp = get_new_intermed_temp test_tracker in
    let _, begin_for_in_inst = build_begin_for_in_op for_in_temp array_temp test_tracker in
    let reassign_inst = build_reassign_op undef_temp for_in_temp test_tracker in
    ...
    let reassign_inst2 = build_reassign_op undef_temp load_hoisted_temp test_tracker in
    let begin_else_inst = build_begin_else test_tracker in
    let end_if_inst = build_end_if test_tracker in
    let print_temp, load_print_inst = build_load_builtin "isNaN" test_tracker in
