...
let correct =
    ...
    let compare_temp_target, load_compare_temp_inst = build_load_integer 2L tracker in
    let compare_temp, compare_inst = build_compare_op integer_temp compare_temp_target LessThan tracker in
    let i_temp, load_i_inital_inst = build_load_integer 0L tracker in
    let begin_while_inst = build_begin_while compare_temp i_temp NotEqual tracker in
    let reassign_hoisted_inst = build_reassign_op undef_temp integer_temp tracker in
    let _, inc_loop_var_inst = build_unary_op integer_temp PostInc tracker in
    let loop_compare_temp, load_loop_temp_inst = build_load_integer 2L tracker in
