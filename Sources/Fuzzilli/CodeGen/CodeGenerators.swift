...
public let CodeGenerators: [CodeGenerator] = [
    ...
    CodeGenerator("WasmModuleGenerator", inContext: .javascript) { b in
        let module = b.buildWasmModule { m in
            let signature = [] => .wasmi32

            if probability(0.2) {
                repeat {
                    let visibleJsWasmGlobals = b.getVisibleJsWasmGlobals()
                    if visibleJsWasmGlobals.isEmpty {
                        break
                    }
                    // Choose a random available global type as an import.
                    let global = chooseUniform(from: visibleJsWasmGlobals)

                    let _ = m.addGlobal(importing: global)
                } while false
            }

            m.addWasmFunction(with: signature) { function, _ in
                let variable = function.consti32(1337)
                function.wasmReturn(variable)
            }
        }
    },
    ...
    CodeGenerator("WasmImportGlobalGenerator", inContext: .wasm) { b in
        ...
        // TODO: I think, if we are in wasm, the wasm imported globals will also have the WasmGlobal.* type and as such this might try to import a variable which originates in Wasm (i.e. through an import). This will fail during lifting as it cannot find a Js expression for this variable (as it lives in the wasm world).
        // Maybe this is fixed as we now convert the js world type to the wasm world type in WasmImportGlobal.
        let visibleJsWasmGlobals = b.getVisibleJsWasmGlobals()
        guard !visibleJsWasmGlobals.isEmpty else {
            return
        }
        let global = chooseUniform(from: visibleJsWasmGlobals)
        let variable = module.addGlobal(importing: global)
    },
    ...
    CodeGenerator("WasmDefineGlobalGenerator", inContext: .wasm) { b in
        ...
        let _ = module.addGlobal(wasmGlobal: wasmGlobal, isMutable: probability(0.5))
    },
    ...
    CodeGenerator("WasmGlobalStoreGenerator", inContext: .wasmFunction) { b in
        ...
        // Try to rely on the typer here.
        if !b.type(of: randomGlobal.0).Is(randomGlobal.1.toType()) {
//            logger.info(" [!] mismatched global info in globalstore generator, relying on stored global info")
        }
    ...
    CodeGenerator("WasmGlobalLoadGenerator", inContext: [.wasmFunction]) { b in
        ...
        let _ = function.wasmLoadGlobal(globalVariable: randomGlobal.0)
    },
    ...
    CodeGenerator("WasmLoadi32Generator", inContext: .wasmFunction) { b in
        let function = b.currentWasmModule.currentWasmFunction
        function.consti32(Int32(truncatingIfNeeded: b.randomInt()))
    },

    CodeGenerator("WasmLoadi64Generator", inContext: .wasmFunction) { b in
        let function = b.currentWasmModule.currentWasmFunction
    ...
    CodeGenerator("Wasmi64BinOpGenerator", inContext: .wasmFunction) { b in
        ...
        let varA = b.randomVariable(ofType: .wasmi64) ?? function.consti64(b.randomInt())
        b.hide(varA)
    ...
    CodeGenerator("WasmFunctionGenerator", inContext: .wasm) { b in
        ...
        let returnType: ILType = withEqualProbability({
            return .wasmi32
        }, {
            return .wasmi64
        }, {
        ...
        module.addWasmFunction(with: signature) { function, _ in
            if !returnType.Is(.nothing) {
                let variable = function.generateRandomWasmVar(ofType: returnType)
                function.wasmReturn(variable)
            } else {
                // Create some variables.
                function.consti64(1337);
                function.consti32(1338);
            }
        }
    },
    ...
    CodeGenerator("WasmReassignmentGenerator", inContext: .wasmFunction, inputs: .oneWasmPrimitive) { b, v in
        ...
        if let varB = b.randomVariable(ofType: b.type(of: v)), b.type(of: varB).Is(.wasmPrimitive) {
            function.wasmReassign(variable: v, to: varB)
        }
    },

    CodeGenerator("WasmFunctionGlobalLoadGenerator", inContext: .wasm) { b in
        let module = b.currentWasmModule

        let randomGlobal = b.randomWasmGlobal()

        let global = module.addGlobal(wasmGlobal: randomGlobal, isMutable: true)

        let signature = [] => randomGlobal.toType()

        module.addWasmFunction(with: signature) { function, _ in
            let variable = function.wasmLoadGlobal(globalVariable: global)
            function.wasmReturn(variable)
        }
    },

    CodeGenerator("WasmFunctionGlobalStoreGenerator", inContext: .wasm) { b in
        let module = b.currentWasmModule
        ...
        module.addWasmFunction(with: signature) { function, _ in
            let variable = function.generateRandomWasmVar(ofType: randomGlobal.toType())
            function.wasmStoreGlobal(globalVariable: global, to: variable)
            function.wasmReturn(variable)
        }
    },
