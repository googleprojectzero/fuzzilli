...
public let ProgramTemplates = [
    ...
    WasmProgramTemplate("ThrowInWasmCatchInJS") { b in
        ...
        let sharedRef = false
        let wasmExnRefType = ILType.wasmExnRef(shared: sharedRef)
        let catchBlockOutputTypes = b.type(of: tagToCatchForRethrow).wasmTagType!.parameters + [wasmExnRefType]
        ...
        let module = b.buildWasmModule { wasmModule in
            ...
            wasmModule.addWasmFunction(with: [] => []) { function, label, args in
                ...
                b.build(n: 10)
                function.wasmBuildThrowRef(exception: b.randomVariable(ofType: wasmExnRefType)!, sharedRef: sharedRef)
                return []
            }
        }
