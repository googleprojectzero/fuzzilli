...
public class Fuzzer {
    ...
    //
    // This will also initialize the fuzzer if necessary and run a number of startup tests.
    ...
    // timers to be scheduled, communication channels to be established, etc.
    // Essentially, the fuzzer thread will be active after initialization.
    public func initialize() {
        ...
        // Install a watchdog to monitor utilization of the master instances.
        if config.isMaster {
    ...
    public func importCorpus(_ corpus: [Program], withDropout doDropout: Bool = false) {
        for program in corpus {
            core.importProgram(program, withDropout: doDropout)
        }
    }
    
    // Import a potentially interesting program into this fuzzer.
    public func importProgram(_ program: Program, withDropout doDropout: Bool = false) {
        core.importProgram(program, withDropout: doDropout)
    }
    ...
    // Construct a new ProgramBuilder for the current fuzzing context.
    public func makeBuilder() -> ProgramBuilder {
        return ProgramBuilder(for: self)
    }
    
    // Make a logger for this fuzzer with the given label.
    ...
    // Construct a non-trivial program for execution time measurement.
