import Foundation
/// Utility for intelligent selection of Mutators.
/// https://github.com/DeamonSpawn/amenezes.github.io/blob/gh-pages/Docs/IntelligentCodeGenerationAndMutationToAidFuzzingOfJavaScriptEngines.pdf
/// The implementation is based on the Multi-Armed Bandit Problem.
/// The algorithm used is based on the EXP3.1 adversarial bandit.
///
/// The utility provides dynamic rebalancing of the weight of a selected mutator in response to coverage generated.
/// Coverage performance for mutators are evaluated over a fixed threshold (critical mass threshold) of fuzzer iterations.
/// The weights of mutators are updated upon reaching the critical mass threshold.
/// Additionally, the weights for mutators are reset after a restart threshold to prevent dominance of a single mutator over the others. 
...
public class MutatorMultiArmedBandit {
    ...
    /// This value returned is only updated after the critical mass threshold.
    ...
    /// These stats are only updated after every critical mass threshold.
