    ...
    func reduce(_ code: inout Code, with verifier: ReductionVerifier) {
        for instr in code {
            ...
            case let op as CallSuperMethod:
                ...
                let newOp = CallSuperMethod(methodName: op.methodName, numArguments: op.numArguments - 1, spreads: op.spreads.dropLast())
                let newInstr = Instruction(newOp, output: instr.output, inputs: Array(instr.inputs.dropLast()))
                verifier.tryReplacing(instructionAt: instr.index, with: newInstr, in: &code)
            ...
            case let op as CallInstanceMethod:
                ...
                let newOp = CallInstanceMethod(methodName: op.methodName, isPrivate: op.isPrivate, numArguments: op.numArguments - 1, spreads: op.spreads.dropLast())
                let newInstr = Instruction(newOp, output: instr.output, inputs: Array(instr.inputs.dropLast()))
                verifier.tryReplacing(instructionAt: instr.index, with: newInstr, in: &code)
