...
public class ProgramBuilder {
    ...
    private func generateWithCodeGen() {
        ...
        /// Selects random codeGenerator with required context
        let generator = fuzzer.selectRandomCodeGenerator(withContext: self.context)

        // The empty code generator is only received if there are no available candidates
        guard generator.name != "Empty" else { return }
    ...
    private func generateInternal(mode: GenerationMode) {
        ...
        while currentCodegenBudget > 0 {
            ...
            switch mode {
                case .spliceOnly:
                    fuzzer.isSpliceMutation = true
                    self.generateWithSplicing()
                case .codeGenOnly:
                    fuzzer.isCodeGeneration = true
