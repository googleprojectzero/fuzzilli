...
EXT_OBJS = $(patsubst Corpus/extra/%.js, corp_temp/ext_%.il.protobuf, $(EXT_SRCS))
...
SPM_OBJS = $(patsubst Corpus/spidermonkey/%.js, corp_temp/spm_%.il.protobuf, $(SPM_EXT_SRCS))

V8_SRCS = $(shell find Corpus/v8 -type f -name "*.js")
V8_OBJS = $(patsubst Corpus/v8/%.js, corp_temp/v8_%.il.protobuf, $(V8_EXT_SRCS))
...
corp_temp/ext_%.il.protobuf: Corpus/extra/%.js
	$(FUZZILLI_COMP) $(FUZZILLI_COMP_OPT) $< $@ > /dev/null
